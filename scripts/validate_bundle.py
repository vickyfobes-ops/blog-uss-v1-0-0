...
def english_body_word_count(text: str) -> int:
    """Count publishable English body words, excluding headings and image markup."""
...
def markdown_metrics(text: str) -> dict[str, Any]:
    ...
    h2 = [match.group(1).strip() for match in h2_matches]
    images = markdown_images(text)
    final_h2 = h2[-1].strip() if h2 else ""
    penultimate_h2 = h2[-2].strip() if len(h2) >= 2 else ""
    faq_part = text[h2_matches[-1].end() :] if h2_matches else ""
    sources_part = ""
    if len(h2_matches) >= 2:
        sources_part = text[h2_matches[-2].end() : h2_matches[-1].start()]
    faqs = re.findall(r"^### (.+)$", faq_part, flags=re.MULTILINE)
    external_links = re.findall(r"\[[^\]]+\]\((https?://[^\)]+)\)", sources_part)
    return {
        "h1": len(h1),
        "h2": len(h2),
        "final_h2": final_h2,
        "penultimate_h2": penultimate_h2,
        "faq": len(faqs),
        "images": len(images),
        "sources_before_faq": len(external_links),
        "english_body_words": english_body_word_count(text),
    }
...
def validate_markdown(label: str, text: str, errors: list[str], warnings: list[str]) -> None:
    ...
    if metrics["penultimate_h2"] not in SOURCE_HEADINGS:
        errors.append(
            f"{label}: H2 immediately before FAQ must be a sources section, "
            f"found {metrics['penultimate_h2']!r}"
        )
    if metrics["faq"] != 5:
    ...
    if metrics["sources_before_faq"] < 3:
        errors.append(
            f"{label}: expected at least 3 external source links in the sources section "
            f"immediately before FAQ, found {metrics['sources_before_faq']}"
        )
    if label == "English Markdown" and metrics["english_body_words"] < MIN_ENGLISH_BODY_WORDS:
        errors.append(
            f"{label}: English body must contain at least {MIN_ENGLISH_BODY_WORDS} words, "
            f"found {metrics['english_body_words']}"
        )
    body = re.sub(r"<!--.*?-->", "", text, flags=re.DOTALL)
...
def validate_meta(
    ...
    content_metrics = meta.get("contentMetrics")
    if not isinstance(content_metrics, dict):
        errors.append("meta: contentMetrics object is required")
    else:
        declared_words = content_metrics.get("englishBodyWords")
        if not isinstance(declared_words, int):
            errors.append("meta: contentMetrics.englishBodyWords must be an integer")
        elif declared_words != actual_english_body_words:
            errors.append(
                "meta: contentMetrics.englishBodyWords must equal the measured English body "
                f"word count ({actual_english_body_words}), found {declared_words}"
            )
        elif declared_words < MIN_ENGLISH_BODY_WORDS:
            errors.append(
                f"meta: contentMetrics.englishBodyWords must be at least {MIN_ENGLISH_BODY_WORDS}"
            )
    ...
    internal_links = meta.get("internalLinkSuggestions")
    if not isinstance(internal_links, list) or len(internal_links) < 3:
        errors.append("meta: internalLinkSuggestions must contain at least 3 items")
    else:
        allowed_link_types = {"product-or-collection", "related-blog", "other"}
        allowed_verification = {"verified", "unverified"}
        link_types: set[str] = set()
        for index, item in enumerate(internal_links, start=1):
            if not isinstance(item, dict):
                errors.append(f"meta: internalLinkSuggestions item {index} must be an object")
                continue
            link_type = str(item.get("targetType", ""))
            link_types.add(link_type)
            if link_type not in allowed_link_types:
                errors.append(
                    f"meta: internalLinkSuggestions item {index} targetType is invalid"
                )
            if not str(item.get("anchorText", "")).strip():
                errors.append(
                    f"meta: internalLinkSuggestions item {index} anchorText is required"
                )
            if item.get("verificationStatus") not in allowed_verification:
                errors.append(
                    f"meta: internalLinkSuggestions item {index} verificationStatus is invalid"
                )
        for required_type in ("product-or-collection", "related-blog"):
            if required_type not in link_types:
                errors.append(
                    f"meta: internalLinkSuggestions must include a {required_type!r} item"
                )

    indexing = meta.get("indexingReadiness")
    if not isinstance(indexing, dict):
        errors.append("meta: indexingReadiness object is required")
    else:
        if indexing.get("canonicalHandleReviewed") is not True:
            errors.append("meta: indexingReadiness.canonicalHandleReviewed must be true")
        incoming = indexing.get("incomingLinkOpportunities")
        if not isinstance(incoming, list) or not any(
            str(item).strip() for item in incoming
        ):
            errors.append(
                "meta: indexingReadiness.incomingLinkOpportunities must contain at least one page"
            )
        for key in ("sitemapCheck", "indexabilityCheck", "searchConsoleRequest"):
            if not str(indexing.get(key, "")).strip():
                errors.append(f"meta: indexingReadiness.{key} is required")
...
def validate_html(label: str, text: str, errors: list[str]) -> None:
    ...
    if len(h2_values) < 2 or h2_values[-2] not in SOURCE_HEADINGS:
        errors.append(f"{label}: HTML H2 immediately before FAQ must be a sources section")
