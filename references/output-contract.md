...
- Put `## Sources and Technical References` or `## 资料来源与技术参考` immediately before FAQ, with at least three external research links and a short explanation of what each supports.
- Use all five FAQ entries for genuine reader questions; do not use a source-note question as an FAQ.
...
- Put the CTA before the sources and FAQ sections.
...
```json
{
  ...
  "internalLinkSuggestions": [
    {
      "targetType": "product-or-collection | related-blog | other",
      "anchorText": "Descriptive proposed anchor",
      "url": "Verified URL, or empty when unverified",
      "verificationStatus": "verified | unverified"
    }
  ],
  "indexingReadiness": {
    "canonicalHandleReviewed": true,
    "incomingLinkOpportunities": [
      "Existing discoverable page that should link to the new article"
    ],
    "sitemapCheck": "Verify after publication",
    "indexabilityCheck": "Verify public 200, crawl permission, no noindex, and self-canonical",
    "searchConsoleRequest": "Request once after live checks pass"
  },
  "imageGeneration": {
    "required": true,
...
Provide at least three `internalLinkSuggestions`: one `product-or-collection`, one `related-blog`, and one additional relevant destination. An unverified URL may remain empty in metadata but must not appear as a live link in publish-ready body copy. `indexingReadiness.incomingLinkOpportunities` must identify at least one existing page that could link to the new canonical URL after publication.
...
- measured English body-word count of at least 1,500; this number must match `contentMetrics.englishBodyWords` exactly;
- internal-link coverage, incoming-link opportunity, and post-publication indexing-readiness checks;
